...
- 移动占位车体与 TF 树闭环：已通过。`VlnVehicleTfPublisher` 发布 `/tf`，当前 TF 树为 `map -> base_link`、`base_link -> front_camera_optical_frame`、`base_link -> lidar_link`；传感器随占位车体沿土路运动，阶段 7 脚本输出 `VLN_VEHICLE_TF_SMOKE_TEST_PASS`。
- 标准化输出闭环：已通过。阶段 8 固定 topic、frame、RViz 配置和 rosbag 记录流程；`/home/ubuntu22/VLN/scripts/run_standardized_outputs_smoke_test.sh` 已输出 `VLN_STANDARDIZED_OUTPUTS_SMOKE_TEST_PASS`，并记录 8 秒标准 rosbag 样本。
...
- 2026-08-14：阶段 7 移动占位车体 + TF 树闭环已通过。新增 `Assets/VLN/Scripts/VlnVehicleTfPublisher.cs`、`/home/ubuntu22/VLN/scripts/ros2_wait_for_vehicle_tf.py`、`/home/ubuntu22/VLN/scripts/ros2_wait_for_camera_info_once.py` 和 `/home/ubuntu22/VLN/scripts/run_vehicle_tf_smoke_test.sh`；通过 run id `vln_vehicle_tf_20260814_010331`，TF 树为 `map -> base_link -> front_camera_optical_frame,lidar_link`，`base_link` 自动验收运动距离约 `0.374m`。随后顺序回归阶段 4/5：图像 `vln_image_20260814_010438` 通过，LiDAR `vln_lidar_20260814_010511` 通过。
- 2026-08-14：阶段 8 标准化输出闭环已通过。新增 `/home/ubuntu22/VLN/scripts/run_standardized_outputs_smoke_test.sh`、`check_standardized_vln_outputs.sh`、`record_vln_sensor_bag_sample.sh`、`view_vln_vehicle_rviz.sh` 和 `/home/ubuntu22/VLN/config/vln_vehicle_sensors.rviz`；自动验收 run id `vln_standardized_outputs_20260814_011059` 输出 `VLN_STANDARDIZED_OUTPUTS_SMOKE_TEST_PASS`。本次 rosbag 位于 `/home/ubuntu22/VLN/VLN_BAGS/vln_standardized_outputs_20260814_011059`，包含 `/vln/front/image_raw` 39 帧、`/vln/front/camera_info` 39 帧、`/vln/lidar/points` 39 帧、`/tf` 78 条消息，大小约 `38.7 MiB`。
