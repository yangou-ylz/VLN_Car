...
- 2026-08-14：新增并验证本地中文控制面板 `/home/ubuntu22/VLN/scripts/vln_control_panel.py`、`start_vln_control_panel.sh`、`vln_control_panel_smoke_client.py` 和 `run_control_panel_smoke_test.sh`；阶段 11 已通过。该阶段使用 Python 标准库 HTTP server、浏览器和已有 ROS2 Python 包，未安装任何新系统包、Python 包或 Conda 包。
- 2026-08-14：新增资产升级工作流文档 `/home/ubuntu22/VLN/docs/asset_upgrade_workflow.md` 和资料索引 `/home/ubuntu22/VLN/VLN_REFERENCE_LIBRARY/asset_candidates/index.md`；当前未下载或导入大型外部资产。
- 2026-08-14：新增资产升级基线检查脚本 `/home/ubuntu22/VLN/scripts/run_asset_upgrade_baseline_check.sh`，用于导入候选地图/小车前后顺序回归标准输出、cmd_vel 控制和中文控制面板；该脚本未引入新依赖。
- 2026-08-14：按用户确认进入阶段 12 第一轮复杂地图导入。使用本地代理 `127.0.0.1:7897` 下载 Kenney Nature Kit 2.1 到 `/home/ubuntu22/VLN/VLN_ASSETS_CACHE/kenney_nature-kit.zip`，下载大小约 `11M`，SHA256 为 `fa7974a0d342bfe63c38664ba9f8ec1a4aab8ea25f099bdc56870e33588c4d9d`；解压缓存位于 `/home/ubuntu22/VLN/VLN_ASSETS_CACHE/kenney_nature-kit_extracted`，约 `36M`。没有安装任何系统包、Python 包或 Conda 包。
- 2026-08-14：从 Kenney Nature Kit 只导入 70 个轻量 FBX 子集到 Unity 工程 `Assets/VLN/ExternalAssets/KenneyNatureKit`，导入后目录约 `2.8M`。导入内容包括树、岩石、石块、灌木、草、栅栏、木桥、木头、营地和标志牌；许可证文件已随工程保存在 `Assets/VLN/ExternalAssets/KenneyNatureKit/Reference/License.txt`，许可证为 Creative Commons Zero / CC0。
- 2026-08-14：新增阶段 12 候选场景 `Assets/VLN/Scenes/VLNOffroadAssetCandidate.unity`，由 `/home/ubuntu22/VLN/UnityProjects/VLN_Offroad/Assets/VLN/Editor/VlnOffroadAssetCandidateProjectSetup.cs` 生成；运行时脚本为 `VlnOffroadAssetCandidateSmokeTest.cs`，自动验收脚本为 `/home/ubuntu22/VLN/scripts/run_offroad_asset_candidate_smoke_test.sh`。候选场景不覆盖主场景，继续使用 `/vln/front/image_raw`、`/vln/front/camera_info`、`/vln/lidar/points`、`/tf` 和 `/vln/cmd_vel`。
- 2026-08-14：阶段 12 候选场景验收通过，`/home/ubuntu22/VLN/scripts/run_offroad_asset_candidate_smoke_test.sh` 输出 `VLN_OFFROAD_ASSET_CANDIDATE_SMOKE_TEST_PASS`，最近通过 run id 为 `vln_offroad_asset_candidate_20260814_033514`。完整资产升级回归 `/home/ubuntu22/VLN/scripts/run_asset_upgrade_baseline_check.sh` 输出 `VLN_ASSET_UPGRADE_BASELINE_CHECK_PASS`，最近通过 run id 为 `vln_asset_baseline_20260814_033514`。
...
- 2026-08-17：扩展本地中文控制面板，新增“速度控制”模块、手动驾驶记录导出和 JSON 回放脚本。新增文件包括 `scripts/replay_manual_drive_recording.py`、`scripts/replay_manual_drive_recording.sh`、`scripts/vln_control_panel_manual_recording_smoke_client.py` 和 `scripts/run_control_panel_manual_recording_smoke_test.sh`；记录目录为 `/home/ubuntu22/VLN/VLN_RECORDINGS/manual_drives`，已加入 `.gitignore`。该阶段只使用 Python 标准库、已有 ROS2 `geometry_msgs` 和已有控制面板运行环境，未安装任何系统包、Python 包或 Conda 包。
