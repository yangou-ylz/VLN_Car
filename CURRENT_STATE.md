...
2026-08-24 Topgear 鱼眼/高频传感器配置已应用到 Mesa Topgear 场景：四路 UnitySensors RGB 相机统一设为 `FOV=120°`、`640x480`、`20Hz`，作为师兄口径“大于 90° 即鱼眼视角”的超广角/鱼眼近似；四路相机额外挂 Unity Post Processing `Lens Distortion`，参数为 `intensity=-55`、`scale=1.08`。LiDAR 已从上一版 `10Hz / 45m / 7200点每帧` 更新为 `16Hz / 90m / 57600点每帧`，其中 `90m` 是原默认最大距离的两倍，`57600` 等于 VLP-16 scan pattern 完整一圈，避免 RViz 只能看到小扇区慢慢扫。RViz 点云显示配置同步改为 `60 FPS`、PointCloud2 `Depth=20`、`Decay Time=1`、点大小 `2px`。自动验收入口为 `scripts/run_mesa_topgear_fisheye_sensor_rate_smoke_test.sh`，最新通过 run id `vln_mesa_topgear_fisheye_sensor_rate_20260824_170436`：前/后/左/右相机实际频率约 `19.837/20.113/20.803/20.072Hz`，LiDAR 实际频率约 `16.080Hz`，结果文件确认 `lidar_target_max_range_m=90.0`、`lidar_applied_points_per_scan=57600`、`lidar_scan_pattern_size=57600`。该改动只调相机 FOV/畸变、UnitySensors 发布频率、LiDAR 最大距离、LiDAR 每帧点数和 RViz 显示配置，不改 Topgear 传感器位姿锁定文件、不改传感器官方模型、不改小车物理或旧 13 点路线。
