...
FISHEYE_CAPTURE_LOG="$LOG_DIR/ros2_fisheye_capture_validate.log"
FISHEYE_CAPTURE_DIR="$LOG_DIR/ros2_fisheye_capture"
...
fisheye_view_angle=$(get_kv_value "$RESULT_FILE" camera_fisheye_view_angle_deg)
...
fisheye_sensor_count=$(get_kv_value "$RESULT_FILE" fisheye_sensor_configured_count)
rgb_sensor_disabled_count=$(get_kv_value "$RESULT_FILE" rgb_sensor_disabled_count)
image_publisher_fisheye_source_count=$(get_kv_value "$RESULT_FILE" image_publisher_fisheye_source_count)
unity_sensors_image_publisher_configured_count=$(get_kv_value "$RESULT_FILE" unity_sensors_image_publisher_configured_count)
extra_image_publisher_disabled_count=$(get_kv_value "$RESULT_FILE" extra_image_publisher_disabled_count)
legacy_vln_fisheye_image_publisher_removed_count=$(get_kv_value "$RESULT_FILE" legacy_vln_fisheye_image_publisher_removed_count)
legacy_camera_info_disabled_count=$(get_kv_value "$RESULT_FILE" legacy_camera_info_publisher_disabled_count)
fisheye_camera_info_count=$(get_kv_value "$RESULT_FILE" fisheye_camera_info_publisher_configured_count)
post_process_disabled_count=$(get_kv_value "$RESULT_FILE" post_process_layer_disabled_count)
fisheye_material_configured=$(get_kv_value "$RESULT_FILE" fisheye_material_configured)
...

{
  ...
  echo "camera_fisheye_view_angle_deg=${fisheye_view_angle:-missing}"
  ...
  echo "fisheye_sensor_configured_count=${fisheye_sensor_count:-missing}"
  echo "rgb_sensor_disabled_count=${rgb_sensor_disabled_count:-missing}"
  echo "image_publisher_fisheye_source_count=${image_publisher_fisheye_source_count:-missing}"
  echo "unity_sensors_image_publisher_configured_count=${unity_sensors_image_publisher_configured_count:-missing}"
  echo "extra_image_publisher_disabled_count=${extra_image_publisher_disabled_count:-missing}"
  echo "legacy_vln_fisheye_image_publisher_removed_count=${legacy_vln_fisheye_image_publisher_removed_count:-missing}"
  echo "legacy_camera_info_publisher_disabled_count=${legacy_camera_info_disabled_count:-missing}"
  echo "fisheye_camera_info_publisher_configured_count=${fisheye_camera_info_count:-missing}"
  echo "post_process_layer_disabled_count=${post_process_disabled_count:-missing}"
  echo "fisheye_material_configured=${fisheye_material_configured:-missing}"
...
if [ "${distortion_enabled:-1}" != "0" ]; then echo "legacy_lens_distortion_should_be_disabled"; exit 1; fi
if [ "${fisheye_sensor_count:-0}" -ne 4 ]; then echo "fisheye_sensor_configured_count_failed"; exit 1; fi
if [ "${rgb_sensor_disabled_count:-0}" -ne 4 ]; then echo "legacy_rgb_sensor_not_disabled"; exit 1; fi
if [ "${image_publisher_fisheye_source_count:-0}" -ne 4 ]; then echo "unity_sensors_image_publisher_not_bound_to_fisheye"; exit 1; fi
if [ "${unity_sensors_image_publisher_configured_count:-0}" -ne 4 ]; then echo "unity_sensors_image_publisher_not_configured"; exit 1; fi
if [ "${legacy_camera_info_disabled_count:-0}" -ne 4 ]; then echo "legacy_camera_info_not_disabled"; exit 1; fi
if [ "${fisheye_camera_info_count:-0}" -ne 4 ]; then echo "fisheye_camera_info_not_configured"; exit 1; fi
if [ "${post_process_disabled_count:-0}" -ne 4 ]; then echo "post_process_layer_not_disabled"; exit 1; fi
if [ "${fisheye_material_configured:-0}" != "1" ]; then echo "fisheye_material_not_configured"; exit 1; fi
if [ "${camera_resolution:-missing}" != "640x640" ]; then echo "fisheye_camera_resolution_failed"; exit 1; fi
...
lidar_freq_pid=$!
timeout 105s bash -lc "$ROS_ENV; python3 /home/ubuntu22/VLN/scripts/ros2_capture_fisheye_images.py --output-dir '$FISHEYE_CAPTURE_DIR' --timeout 100 --width 640 --height 640 --encoding rgb8 --view-angle-deg 190 --rectified-fov-deg 90" >"$FISHEYE_CAPTURE_LOG" 2>&1 &
fisheye_capture_pid=$!
...
if [ "$unity_status" -ne 0 ]; then
  kill "$front_freq_pid" "$rear_freq_pid" "$left_freq_pid" "$right_freq_pid" "$lidar_freq_pid" "$fisheye_capture_pid" "$topic_pid" >/dev/null 2>&1 || true
fi
...
wait "$fisheye_capture_pid"; fisheye_capture_status=$?
...

{
  ...
  echo "fisheye_capture_status=$fisheye_capture_status"
  ...
  echo "ros2_fisheye_capture_dir=$FISHEYE_CAPTURE_DIR"
...
echo "ROS2 鱼眼图像采集和反校正："
sed -n '1,220p' "$FISHEYE_CAPTURE_LOG" || true
...
if [ "$fisheye_capture_status" -ne 0 ]; then echo "ros2_fisheye_capture_validation_failed"; exit 1; fi
