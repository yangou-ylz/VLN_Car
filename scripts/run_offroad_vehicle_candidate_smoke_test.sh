...
# 阶段 12：真实小车视觉候选场景自动验收。
# 当前候选：Clearpath Husky 视觉 mesh 子集 + Kenney 越野候选地图 + 现有 VLN 传感器/TF/控制接口。

set -eo pipefail

VLN_ROOT="/home/ubuntu22/VLN"
UNITY_PROJECT="$VLN_ROOT/UnityProjects/VLN_Offroad"
RUN_ID="vln_offroad_vehicle_candidate_$(date +%Y%m%d_%H%M%S)"
...
RESULT_FILE="$UNITY_PROJECT/Logs/vln_offroad_vehicle_candidate_result.txt"
SCREENSHOT_FILE="$UNITY_PROJECT/Logs/vln_offroad_vehicle_candidate_screenshot.png"
DETAIL_SCREENSHOT_FILE="$UNITY_PROJECT/Logs/vln_offroad_vehicle_candidate_detail_screenshot.png"
...
if [ -f "$RESULT_FILE" ]; then
  mv "$RESULT_FILE" "$LOG_DIR/previous_vln_offroad_vehicle_candidate_result.txt"
fi

if [ -f "$SCREENSHOT_FILE" ]; then
  mv "$SCREENSHOT_FILE" "$LOG_DIR/previous_vln_offroad_vehicle_candidate_screenshot.png"
fi

if [ -f "$DETAIL_SCREENSHOT_FILE" ]; then
  mv "$DETAIL_SCREENSHOT_FILE" "$LOG_DIR/previous_vln_offroad_vehicle_candidate_detail_screenshot.png"
fi
...
timeout 95s bash -lc "$ROS_ENV; python3 /home/ubuntu22/VLN/scripts/ros2_wait_for_image_once.py --topic $IMAGE_TOPIC --width 1280 --height 720 --encoding rgb8 --frame-id front_camera_optical_frame --timeout 90" >"$IMAGE_LOG" 2>&1 &
image_pid=$!

timeout 95s bash -lc "$ROS_ENV; python3 /home/ubuntu22/VLN/scripts/ros2_wait_for_camera_info_once.py --topic $CAMERA_INFO_TOPIC --width 1280 --height 720 --frame-id front_camera_optical_frame --timeout 90" >"$CAMERA_INFO_LOG" 2>&1 &
camera_info_pid=$!
...
set +e
timeout 140s "$VLN_ROOT/scripts/open_unity_vln_project.sh" \
  -batchmode \
  -executeMethod VLN.Editor.VlnOffroadVehicleCandidateSmokeTestRunner.Run \
  -logFile "$UNITY_LOG"
unity_status=$?
...
if [ -f "$SCREENSHOT_FILE" ]; then
  cp "$SCREENSHOT_FILE" "$LOG_DIR/vln_offroad_vehicle_candidate_screenshot.png"
fi

if [ -f "$DETAIL_SCREENSHOT_FILE" ]; then
  cp "$DETAIL_SCREENSHOT_FILE" "$LOG_DIR/vln_offroad_vehicle_candidate_detail_screenshot.png"
fi

model_count=$(find "$UNITY_PROJECT/Assets/VLN/ExternalAssets/HuskyVisual/Models" -maxdepth 1 -type f -name '*.dae' 2>/dev/null | wc -l)

{
  ...
  echo "vehicle_model_count=$model_count"
...
echo "Unity vehicle candidate result:"
sed -n '1,160p' "$RESULT_FILE" 2>/dev/null || true
echo "Key Unity log lines:"
grep -n -E "VLN_OFFROAD_VEHICLE_CANDIDATE|Incompatible protocol|Exception|NullReference|error CS|Compilation failed|SocketException|Connection.*failed|Successfully connected|Exiting" "$UNITY_LOG" | sed -n '1,340p' || true
...
if [ ! -s "$SCREENSHOT_FILE" ]; then
  echo "vehicle_candidate_screenshot_missing"
  exit 1
fi

if [ ! -s "$DETAIL_SCREENSHOT_FILE" ]; then
  echo "vehicle_candidate_detail_screenshot_missing"
  exit 1
fi

if [ "$model_count" -lt 5 ]; then
  echo "vehicle_candidate_husky_mesh_subset_incomplete"
  exit 1
fi

echo "VLN_OFFROAD_VEHICLE_CANDIDATE_SMOKE_TEST_PASS"
